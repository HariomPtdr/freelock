...
export default function InterviewRoom() {
  ...
  useEffect(() => {
    ...
    socket.emit('join-interview', meetingRoomId)

    socket.on('receive-message', (msg) => {
      setMessages(prev => [...prev, msg])
    })

    socket.on('user-typing', () => setOtherTyping(true))
    socket.on('user-stop-typing', () => setOtherTyping(false))
  ...
  const sendMessage = () => {
    if (!text.trim()) return
    socketRef.current?.emit('send-interview-message', {
  ...
  const handleTyping = (e) => {
    ...
    if (!typing) {
      setTyping(true)
      socketRef.current?.emit('typing', { contractId: meetingRoomId, name: user.name })
    }
  ...
  const stopTyping = () => {
    setTyping(false)
    socketRef.current?.emit('stop-typing', { contractId: meetingRoomId })
    clearTimeout(typingTimerRef.current)
  }
  ...
  const acceptCall = async () => {
    try {
      ...
      peer.on('signal', (signal) => {
        socketRef.current?.emit('accept-call', { contractId: meetingRoomId, signal })
      })
  ...
  const endCall = (notify = true) => {
    if (notify) socketRef.current?.emit('end-call', { contractId: meetingRoomId })
